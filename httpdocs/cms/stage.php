							...
							<table class="table table-hover">
								...
								<tbody>
									...
									if($_SERVER["REQUEST_METHOD"] == "POST") {
										$deleteId = encodeString($_POST["stageId"]);
										
										require 'php/private/prepare.php';

										if ($stmt = $mysqli->prepare("DELETE FROM solicitatie WHERE id=?")) {
											$stmt->bind_param("i", $deleteId);
											$stmt->execute();
											showError("Success", "De geselecteerde stage is successvol verwijderd.");
											$stmt->close();
										}
									...
									if ($stmt = $mysqli->prepare("SELECT id, email, functie, cv, motivatie FROM solicitatie")) {
										$stmt->execute();
										$stmt->bind_result($stageId, $email, $functie, $cv, $motivatie);
										
										while ($stmt->fetch()) {
											echo "<tr><form method='post'><input type='hidden' name='stageId' value='" . $stageId . "'><td>" . $email . "</td><td><a href='../images/cv/" . $cv . "'>Download</a></td><td>" . $functie . "</td><td><pre>" . $motivatie . "</pre></td><td><input type='submit' value='Verwijder'></td></form></tr>";
										}
