<?php if(LOGGED_IN) {
    ...
    if(!empty($_POST['editPersoneel'])){

        $personeel->update(array('naam' => $_POST['naam'], 'text' => $_POST['text']),array('id'=>$_POST['id']));
    }
...
padding: 0 20px;
        ...
        echo '<table class="portfolios">';
        while ($row = $portfolios->fetch(PDO::FETCH_ASSOC)){
        echo '<tr><form method="post"><input type="hidden" name="id" value="'.$row['id'].'" ><td>'.$row['titel'].'</td><td><img height="100" src="images/thumbnail/'.$row['filename'].'"></td><td><input type="submit" value="verwijder" name="deleteWerk"></td></form></tr>';
        }
        echo '</table>';
    }
    //admin content
    ...
    foto<input style="margin-left: -11px" required name="image" type="file" accept="image/*">
...
<div class="col-md-3 col-sm-6 col-xs-12 col-padding">
                            <div class="wrap">
                                ...
                                <input required name="naam" type="text" placeholder="naam" value="' . $persoon['naam'] . '">
                                <textarea required name="text" placeholder="Beschrijving van de persoon">' . $persoon['text'] . '</textarea>
                   
                                <input type="hidden" name="id" value="' . $persoon['id'] . '">
                                <input type="submit" name="deletePersoon" value="verwijderen"><input name="editPersoneel" type="submit" value="aanpassen"></form>
                            </div>
                        </div>';
            }
            echo '</div></div></div>';
            ...
            echo '<div style="margin: 0 auto" id="reviews">';
    ...
    <div class="reviewname3">
        ' . $row['comment'] . '
            ...
            echo '</div>';
            echo '<style>.star2.active .stararea, .star.active {
    color: #f1c40f;
}</style>';
